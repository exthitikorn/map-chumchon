...
เปิดไฟล์ `index.html` ตรงๆ ในเบราว์เซอร์ **ไม่ได้** — ต้องรันผ่านเว็บเซิร์ฟเวอร์เพราะแอปโหลดข้อมูลจากไฟล์ JSON และใช้ PHP API สำหรับบันทึกหมุด/อัปโหลดรูป

### Docker (แนะนำ — รองรับ PHP API ครบ)

ต้องมี [Docker Desktop](https://www.docker.com/products/docker-desktop/) หรือ Docker Engine ติดตั้งแล้ว
...
หยุดเซิร์ฟเวอร์: `Ctrl+C` แล้วรัน `docker compose down`

ข้อมูลหมุด (`data/community-pins.json`) และรูปที่อัปโหลด (`images/`) จะเก็บบนเครื่อง host ผ่าน volume — ไม่หายเมื่อหยุด container

### เซิร์ฟเวอร์ท้องถิ่น (ไม่มี PHP)

ถ้ารันแบบ static server อย่างเดียว API บันทึกหมุดจะใช้ไม่ได้ แต่ดูแผนที่ได้
